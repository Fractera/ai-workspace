...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS media (
    id          TEXT PRIMARY KEY,
    name        TEXT NOT NULL,           -- original filename (e.g. photo.jpg)
    title       TEXT DEFAULT '',         -- user-defined human label
    description TEXT DEFAULT '',         -- user-defined description
    url         TEXT NOT NULL,           -- full public URL to file
    mime_type   TEXT NOT NULL,           -- e.g. image/jpeg, video/mp4
    extension   TEXT NOT NULL,           -- e.g. jpg, mp4
    crop_mode   TEXT DEFAULT '',         -- horizontal | square | vertical (images only)
    size        INTEGER NOT NULL,        -- bytes
    width       INTEGER,                 -- pixels (images only)
    height      INTEGER,                 -- pixels (images only)
    duration    REAL,                    -- seconds (video only)
    storage_key TEXT NOT NULL UNIQUE,    -- relative path inside storage/
    created_at  TEXT NOT NULL DEFAULT (datetime('now'))
  )
`)

// Safe migrations for existing DBs
const existingCols = db.prepare('PRAGMA table_info(media)').all().map(c => c.name)
if (!existingCols.includes('title'))       db.exec(`ALTER TABLE media ADD COLUMN title TEXT DEFAULT ''`)
if (!existingCols.includes('description')) db.exec(`ALTER TABLE media ADD COLUMN description TEXT DEFAULT ''`)
if (!existingCols.includes('url'))         db.exec(`ALTER TABLE media ADD COLUMN url TEXT NOT NULL DEFAULT ''`)
if (!existingCols.includes('crop_mode'))   db.exec(`ALTER TABLE media ADD COLUMN crop_mode TEXT DEFAULT ''`)
...
app.post('/media/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const baseUrl = process.env.NEXT_PUBLIC_MEDIA_URL ?? `http://localhost:${PORT}`
    const row = {
      id,
...
app.patch('/media/:id', (req, res) => {
  ...
  db.prepare('UPDATE media SET title = ?, description = ?, crop_mode = ? WHERE id = ?')
