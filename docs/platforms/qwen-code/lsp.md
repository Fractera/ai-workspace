...
- **Check if the server is installed**: Run the command manually to verify
- **Check the PATH**: Ensure the server binary is in your system PATH
- **Check workspace trust**: The workspace must be trusted for LSP
- **Check logs**: Look for error messages in the console output
- **Verify `--experimental-lsp` flag**: Make sure you're using the flag when starting Qwen Code

### Slow Performance

- **Large projects**: Consider excluding `node_modules` and other large directories
- **Server timeout**: Increase `startupTimeout` in server configuration for slow servers

### No Results

- **Server not ready**: The server may still be indexing
- **File not saved**: Save your file for the server to pick up changes
...
- **Use project-specific settings**: Configure server options per project when needed via `.lsp.json`
- **Keep servers updated**: Update your language servers regularly for best results
