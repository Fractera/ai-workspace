...
> **Note:** Some sandbox-related environment variables may have used the `GEMINI_*` prefix historically. All new environment variables use the `QWEN_*` prefix.
...
> **Important:** If `QWEN_SANDBOX` is set, it overrides the CLI flag and `settings.json`.
...
> **Tip:** Start with `permissive-open`, then tighten to `restrictive-closed` if your workflow still works.
...
Example (Podman) — to disable SELinux labeling for volume mounts:
...
For a working allowlist-style proxy example, see: Example Proxy Script.
...
For more details on customizing the sandbox, see Customizing the sandbox environment.
