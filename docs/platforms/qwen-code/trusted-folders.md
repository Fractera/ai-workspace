...
- **Workspace Settings are Ignored**: The CLI will not load the `.qwen/settings.json` file from the project. This prevents the loading of custom tools and other potentially dangerous configurations.
- **Environment Variables are Ignored**: The CLI will not load any `.env` files from the project.
