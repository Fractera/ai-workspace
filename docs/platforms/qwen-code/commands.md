...
> **Tip:** Use `/btw` when you need a quick answer without derailing your main task. It's especially useful for clarifying concepts, checking facts, or getting quick explanations while staying focused on your primary workflow.
...
> **Tip:** These commands run outside of a Qwen Code session. Use them to configure authentication before starting a session, or in scripts and CI environments. See the Authentication page for full details.
...
| Standalone `@` | Used when discussing the `@` symbol itself | `@ What is this symbol used for in programming?` |
...
| Function | Description | Priority | Applicable Scenarios |
|---|---|---|---|
| Namespace | Subdirectory creates colon-named commands | — | Better command organization |
| Global Commands | `~/.qwen/commands/` | Low | Personal frequently used commands, cross-project use |
| Project Commands | `<project root>/.qwen/commands/` | High | Team sharing, project-specific commands |

**Priority Rules:** Project commands > User commands (project command used when names are the same)
...
> **Warning:** TOML format is still supported but will be removed in a future version. Please migrate to Markdown format.
...
| Shell Command Injection | `!{command}` | Need dynamic content | Execution confirmation required |

#### 1. Context-aware Injection (`{{args}}`)

| Scenario | Configuration | Call Method | Actual Effect |
|---|---|---|---|
...
|---|---|---|
| Text Files | Supported | Directly inject content |
| Images/PDF | Supported (multi-modal) | Encode and inject |
| Binary Files | Limited | May be skipped or truncated |
| Directory | Supported | Recursive injection, follows `.gitignore` rules |
...
**"Pure Function Refactoring" Command — Creation Steps**
...
| 3. Edit command content | See file below |
