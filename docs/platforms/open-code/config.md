...
- `cors` — Additional origins to allow for CORS when using the HTTP server from a browser-based client. Values must be full origins (scheme + host + optional port), e.g. `https://app.example.com`.
...
If you don't want updates but want to be notified when a new version is available, set `autoupdate` to `"notify"`. Note that this only works if it was not installed using a package manager such as Homebrew.
