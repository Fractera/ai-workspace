...
in the background and turns recurring workflows into reusable Agent Skills. You
review, accept, or discard each extracted skill before it becomes available to
future sessions.
...
Auto Memory complements—but does not replace—the `save_memory` tool, which
captures single facts into `GEMINI.md`. Auto Memory captures multi-step
...
2. **Lock acquisition.** A lock file in the project's memory directory
   ...
   a time.
3. **Sub-agent extraction.** A specialized sub-agent (named `confucius`) reviews
   the session index, reads any sessions that look like they contain repeated
   procedural workflows, and drafts new `SKILL.md` files. Its instructions tell
   it to default to creating zero skills unless the evidence is strong, so most
...
- Auto Memory does not extract skills from the current session. It only considers
  sessions that have been idle for three hours or more.
- Inbox items are stored per project. Skills extracted in one workspace are not
  visible from another until you promote them to the user-scope skills directory.
...
- Explore the memory management tutorial for the complementary `save_memory` and `GEMINI.md` workflows.
- Review the experimental settings catalog in Settings.
