...
> **TIP:** Looking for a high-level comparison of all available subscriptions?
> To compare features and find the right quota for your needs, see our Plans page.
...
- **Individual Google accounts:** Includes all free tier accounts such as
...
- **Organization accounts:** Accounts using paid licenses through an
  organization such as a company, school, or Google Workspace. Includes
...
For instructions, see [Set your Google Cloud Project](#set-your-google-cloud-project).
...
1. Create a service account and key and download the provided JSON file. Assign
...
1. Obtain a Google Cloud API key from the Vertex AI documentation.
...
4. Configure your environment variables. Set either the `GOOGLE_CLOUD_PROJECT`
   ...
   For example, to set the `GOOGLE_CLOUD_PROJECT` variable:
...
authentication credential is cached.
