...
Model steering is particularly useful during complex Plan Mode workflows or
long-running subagent executions where you want to ensure the agent stays on the
right track.
...
enable it using the `/settings` command or by updating your `settings.json` file.
