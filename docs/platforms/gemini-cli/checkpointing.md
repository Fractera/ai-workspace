...
The Checkpointing feature is disabled by default. To enable it, edit your
`settings.json` file.

> **CAUTION:** The `--checkpointing` command-line flag was removed in version
...
Once enabled, checkpoints are created automatically. To manage them, use the
`/restore` command.

### List available checkpoints

To see a list of all saved checkpoints for the current project, run:

```
/restore
```

The CLI will display a list of available checkpoint files. File names are
